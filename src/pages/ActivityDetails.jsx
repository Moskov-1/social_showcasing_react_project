...
export const ActivityDetails = () => {
    const {id} = useParams();
    const activity = AcitvitiesData.find( act => act.id === parseInt(id));
    ...
    return (
        <div>
            ...
            <p className="text-gray-500 mb-6">{activity.date} • <span className="font-bold">{activity.status}</span></p>
            <p className="text-lg leading-relaxed text-gray-700">{activity.description}</p>
        </div>
    );
}